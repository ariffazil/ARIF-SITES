/**
 * USD/MYR Ringgit FX Data API Server — WEALTH Organ
...
const SCRIPT = path.join(__dirname, 'fetch_usdmyr.py');
  ...
  return new Promise((resolve, reject) => {
    execFile(PYTHON, [SCRIPT, command, ...args], {
      ...
      if (err) {
        ...
        console.error(`[usdmyr-api] Python error (${command}):`, msg.trim());
...
const handlers = {
  ...
  '/api/usdmyr/snapshot': async () => {
    ...
    const unsigned = {
      ...
      asset: 'usdmyr',
      observed_at,
      source: 'yfinance + technical analysis (WEALTH FX engine)',
      ticker: {
        ...
        stress_band: ticker.stress_band, mean_90d: ticker.mean_90d,
        stale: false, stale_age_s: 0,
      },
      levels: levels && !levels.error ? {
        support: levels.support_1d || [], resistance: levels.resistance_1d || [],
      ...
      macro: macro && !macro.error ? {
        ...
        sgd_myr: macro.sgd_myr, usd_sgd: macro.usd_sgd,
        usd_idr: macro.usd_idr, usd_thb: macro.usd_thb, usd_cny: macro.usd_cny,
        opr_pct: macro.opr_pct, opr_status: macro.opr_status,
        fed_funds_pct: macro.fed_funds_pct, fed_status: macro.fed_status,
        rate_differential_pct: macro.rate_differential_pct,
      } : {},
    };
    const deepSort = (obj) => {
  ...
  '/api/apex': async () => handlers['/api/usdmyr/apex'](),
  '/api/signal_v2': async () => handlers['/api/usdmyr/signal_v2'](),
  '/api/calendar': async () => handlers['/api/usdmyr/calendar'](),
  '/api/proxies': async () => {
    const c = getCache('proxies'); if (c) return c;
    const d = await runPython('proxies'); setCache('proxies', d); return d;
  },
  '/api/snapshot': async () => handlers['/api/usdmyr/snapshot'](),
  '/api/macro': async () => {
  ...
  '/api/history': async (req, res, params) => handlers['/api/usdmyr/history'](req, res, params),
  '/api/signals': async () => {
    const c = getCache('signals'); if (c) return c;
    const d = await runPython('signal_v2'); setCache('signals', d); return d;
  },
  '/api/levels': async () => {
    const c = getCache('levels'); if (c) return c;
    const d = await runPython('levels'); setCache('levels', d); return d;
  },
  '/api/forecast': async (req, res, params) => handlers['/api/usdmyr/forecast'](req, res, params),
  '/api/usdmyr/forecast': async (req, res, params) => {
  ...
  '/api/usdmyr/ticker': async () => {
    const c = getCache('ticker'); if (c) return c;
    const d = await runPython('ticker'); setCache('ticker', d); return d;
  },
  '/api/usdmyr/history': async (req, res, params) => {
  ...
  '/api/usdmyr/levels': async () => {
    const c = getCache('levels'); if (c) return c;
    const d = await runPython('levels'); setCache('levels', d); return d;
  },
  '/api/usdmyr/macro': async () => {
    const c = getCache('macro'); if (c) return c;
    const d = await runPython('macro'); setCache('macro', d); return d;
  },
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === '/health' || parsed.pathname === '/api/usdmyr/health') {
    const now = new Date().toISOString();
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({
      status: 'ok', uptime: (Date.now() - startTime) / 1000,
      timestamp: now, observed_at: now,
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`[usdmyr-api] USD/MYR FX API running on http://127.0.0.1:${PORT}`);
  console.log(`[usdmyr-api] Endpoints:`);
