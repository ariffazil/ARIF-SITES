/**
 * FBM KLCI Bursa Malaysia Data API Server — WEALTH Organ
...
const SCRIPT = path.join(__dirname, 'fetch_klci.py');
  ...
  return new Promise((resolve, reject) => {
    execFile(PYTHON, [SCRIPT, command, ...args], {
      ...
      if (err) {
        ...
        console.error(`[klci-api] Python error (${command}):`, msg.trim());
...
const handlers = {
  ...
  '/api/klci/snapshot': async () => {
    ...
    const unsigned = {
      ...
      asset: 'klci',
      observed_at,
      source: 'yfinance + technical analysis (WEALTH bursa engine)',
      ticker: {
      ...
      macro: macro && !macro.error ? {
        ...
        usmyr: macro.usmyr, ewm: macro.ewm, sti: macro.sti, hsi: macro.hsi,
  ...
  '/api/apex': async () => handlers['/api/klci/apex'](),
  '/api/signal_v2': async () => handlers['/api/klci/signal_v2'](),
  '/api/calendar': async () => handlers['/api/klci/calendar'](),
  '/api/proxies': async () => {
    const c = getCache('proxies'); if (c) return c;
    const d = await runPython('proxies'); setCache('proxies', d); return d;
  },
  '/api/snapshot': async () => handlers['/api/klci/snapshot'](),
  '/api/macro': async () => {
  ...
  '/api/history': async (req, res, params) => handlers['/api/klci/history'](req, res, params),
  '/api/signals': async () => {
  ...
  '/api/forecast': async (req, res, params) => handlers['/api/klci/forecast'](req, res, params),
  '/api/klci/forecast': async (req, res, params) => {
  ...
  '/api/klci/ticker': async () => {
    const c = getCache('ticker'); if (c) return c;
    const d = await runPython('ticker'); setCache('ticker', d); return d;
  },
  '/api/klci/history': async (req, res, params) => {
  ...
  '/api/klci/levels': async () => {
    const c = getCache('levels'); if (c) return c;
    const d = await runPython('levels'); setCache('levels', d); return d;
  },
  '/api/klci/macro': async () => {
    const c = getCache('macro'); if (c) return c;
    const d = await runPython('macro'); setCache('macro', d); return d;
  },
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname === '/health' || parsed.pathname === '/api/klci/health') {
...
server.listen(PORT, '127.0.0.1', () => {
  console.log(`[klci-api] KLCI Bursa Data API running on http://127.0.0.1:${PORT}`);
  console.log(`[klci-api] Endpoints:`);
