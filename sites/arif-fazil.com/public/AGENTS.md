# Agents Interface Protocol — arif-fazil.com

> Actions AI agents can take on this site (Part of `SEAL_SESSION_arif-2026-06-27-001`)

## FILE GOVERNANCE MODE: FAIL-CLOSED (2026-08-01 · F13)

**Before any write, read:** `https://arif-fazil.com/canon/file-authority.yaml`

- You may not create files outside `forge_work/proposals/<agent-id>/<mission>/` or `receipts/<agent-id>/`.
- You may not modify CANON files unless a lease is explicitly granted by ARIF.
- Every target file must declare authority: **CANON / DERIVED / SCRATCH / PROPOSAL / RECEIPT / RETIRED / UNKNOWN**.
- `UNKNOWN` → stop. `DERIVED` → find upstream SOT. `CANON` → request lease.
- Do not create alternate Atlas, design, routing, or README files. Patch existing authority or propose promotion.
- No execution without receipt. No automation without reversibility.

## Site Identity

- **Name:** arif-fazil.com — Ψ SOUL (Human Surface)
- **Owner:** Muhammad Arif bin Fazil — F13 SOVEREIGN
- **Purpose:** Portfolio, identity, essays, constitutional canon, federation overview

## Available Actions

### Read-Only (No Approval Required)

- Read essays at `/essays/`
- Read canon at `/canon/`
- Read portfolio at `/`
- Read wealth briefing at `/wealth/`
- Read discovery notes at `/discoveries/`
- Access `.well-known/` for agent discovery files
- Access `llms.txt` for site description
- Access `robots.txt` for crawl rules

### WebMCP Tools (Browser-Side)

- `get_soul_identity` — Retrieve Arif's identity and sovereignty proof
- `get_constitutional_floors` — List F1-F13 constitutional floors
- `get_federation_overview` — Get federation organ summary

### MCP Endpoints

- **arifOS Kernel:** `https://mcp.arif-fazil.com/mcp` (POST, JSON-RPC 2.0)
- **GEOX:** `https://geox.arif-fazil.com/mcp` (POST, JSON-RPC 2.0)
- **WEALTH:** `https://wealth.arif-fazil.com/mcp` (POST, JSON-RPC 2.0)
- **WELL:** `https://well.arif-fazil.com/mcp` (POST, JSON-RPC 2.0)

### Restricted (Requires Human Approval)

- Any form submission
- Any state mutation
- Any data upload

## Discovery Files

| File | Path | Purpose |
| --- | --- | --- |
| llms.txt | `/llms.txt` | Site description for AI crawlers |
| status.json | `/status.json` | Live runtime state, organ endpoints & floor compliance |
| robots.txt | `/robots.txt` | Crawl rules |
| sitemap.xml | `/sitemap.xml` | Page index |
| .well-known/webmcp.json | `/.well-known/webmcp.json` | WebMCP browser tool manifest |
| .well-known/agent.json | `/.well-known/agent.json` | Agent card |

## Constitutional Floors

All actions on this site are governed by F1-F13:

- F1 AMANAH — Trust, custody, fiduciary duty
- F2 TRUTH — Epistemic honesty, evidence-grounded claims
- F4 CLARITY — Entropy reduction, signal over noise
- F6 MARUAH — Dignity, harm avoidance
- F9 ANTI-HANTU — No metaphysical claims, no consciousness claims
- F13 SOVEREIGN — Human veto is absolute

## Contact

- **Human:** arif@arif-fazil.com
- **Telegram:** `@ariffazil`
- **GitHub:** `https://github.com/ariffazil`

---

*DITEMPA BUKAN DIBERI — Forged, Not Given*

