...
// Reads from nusantara_atlas.geojson structure.
// One skeleton, two lenses: power (geopolitics) and meaning (linguistics).
// Seven invariant verbs: specify · hold · align · resonate · witness · reclaim · refract.
//
// F2 TRUTH: this file mirrors the canonical GeoJSON. Drift = entropy.
// Nusantara's blindness MUST appear on BOTH faces (honesty constraint).

export interface AxisPowerData {
  sees: string;
  thesis: string;
  gift: string;
  blindness: string;
...
export interface AxisMeaningData {
  sees: string;
  thesis: string;
  gift: string;
  blindness: string;
...
export interface AxisEntry {
  ...
  verb: string;
  power: AxisPowerData;
  meaning: AxisMeaningData;
  mirrorOf: number;
}

const RAW: AxisEntry[] = [
  {
    id: 'V1', short: 'ATLANTIC', color: '#3b82f6', verb: 'specify', mirrorOf: 2,
    power: {
      sees: 'rules-based sea order',
      ...
      blindness: 'reification — believes language describes reality rather than creating it',
  ...
  {
    id: 'V2', short: 'HEARTLAND', color: '#ef4444', verb: 'hold', mirrorOf: 1,
    power: {
    ...
    meaning: {
      ...
      exemplars: ['Bakhtin (The Dialogic Imagination, 1930s)', 'Dostoevsky (the untranslatable soul)', 'Russian toska — no direct English equivalent'],
    },
  },
  {
    id: 'V3', short: 'SINIC', color: '#eab308', verb: 'align', mirrorOf: 4,
    power: {
      ...
      blindness: 'rigidity — the mandate flows down, never bubbles up',
  ...
  {
    id: 'V4', short: 'BHARAT', color: '#f97316', verb: 'resonate', mirrorOf: 3,
    power: {
    ...
    meaning: {
      ...
      gift: 'language as physics — meaning has layers: literal (vācyārtha), suggested (dhvani), meaning from the space between words',
  ...
  {
    id: 'V5', short: 'DAR', color: '#10b981', verb: 'witness', mirrorOf: 6,
    power: {
  ...
  {
    id: 'V6', short: 'GONDWANA', color: '#d97706', verb: 'reclaim', mirrorOf: 5,
    power: {
  ...
  {
    id: 'V7', short: 'NUSANTARA', color: '#06b6d4', verb: 'refract', mirrorOf: 0,
    power: {
    ...
    meaning: {
      ...
      blindness: 'mistakes being passed-through for doing the seeing — the strait doesn\'t understand the ships, it just touches all of them. The default, not the only.',
