---
title: DESIGN_PRIMER
version: PRIMER-1.0-DRAFT
status: DRAFT for web-canon ratification (2026-08-01)
intended_home: canon/design-tokens.json (JSON twin) + canon/design-primer.md (this document)
...
> **arif-fazil.com · Human side (cognitive contrast) + Agent side (structural contrast)**
>
> **Status:** draft for web-canon ratification · 2026-08-01
> **Intended home:** `canon/design-tokens.json` + `canon/design-primer.md`
...
```
REST     translateY(0)    shadow: 0 2px 0 family-700        (the keycap sits proud)
HOVER    translateY(-1px) shadow: 0 3px 0 family-700        (finger pressure, key rises to meet)
ACTIVE   translateY(2px)  shadow: 0 0 0 + inset 0 2px 4px   (full travel, seat the contact)
...
## Appendix A — Diff vs existing `design-tokens.json` (PRIMER-1 vs existing canon)

> Generated by `kimi-code/FI-008` on 2026-08-01 via ATLAS-1.1 ratification
> workflow. Per I8 (diff before mutation), this is the explicit diff
> surfaced for sovereign ratification BEFORE any token values change.

### Colors that CHANGED (PRIMER-1 → existing `design-tokens.json`)

| Token | PRIMER-1 | Existing canon | Notes |
|---|---|---|---|
| y-500 (human primary) | #D9A62E | #FFCC00 (yellow-500) | NEW: amber tone, less neon |
| y-300 | #F2D98C | #FFD54F (yellow-300) | NEW: softer amber |
| y-100 | #FBF3D9 | #FFFAEB (yellow-50) | NEW: paper-sand |
| y-700 | #8F6A14 | #B8860B (yellow-700) | NEW: deep honey |
| y-900 | #4A3608 | #5C4500 (yellow-900) | NEW: umber |
| b-500 (institution) | #2E5F8A | #1E3A8A (blue-500) | NEW: marine, less navy |
| b-300 | #9DB8CE | #91B0F2 (blue-300) | NEW: steel |
| b-700 | #1B3A57 | #152C6B (blue-700) | NEW: deep marine |
| b-900 | #0C1F31 | #0C1A45 (blue-900) | NEW: abyss |
| e-500 (earth) | #2E8A70 | #00D4AA (teal-500) | NEW: viridian, less saturated |
| e-300 | #8CC3B2 | #4FD1C5 (teal-300) | NEW: reef |
| e-700 | #17584A | #047857 (teal-700) | NEW: basin |
| e-900 | #0A2E27 | #064E3B (teal-900) | NEW: trench |
| e-100 | #DFF0EA | #E6FFFA (teal-50) | NEW: shallow |
| r-500 (sovereign) | #B3362B | #E63946 (red-500) | NEW: signal red, deeper |
| r-300 | #D97B6C | #F8A5AE (red-300) | NEW: clay |
| r-700 | #7A1F18 | #A4161A (red-700) | NEW: maroon |
| r-900 | #3D0E0A | #590D14 (red-900) | NEW: deep scar |
| ink | #1A1712 | #1A1A1A | Different shade |
...
| Property | PRIMER-1 | Existing canon | Notes |
|---|---|---|---|
| Travel | 2px | 2px | SAME |
| Timing | 90ms | 90ms | SAME |
| Easing | cubic-bezier(.2,.9,.3,1) | cubic-bezier(0.4, 0, 0.2, 1) | DIFFERENT easing (snappier-out vs material) |
| human radius | 12px | 12px (human_toroidal) | SAME |
| machine radius | 2px | 2px (agent_orthogonal) | SAME |
| Family | IBM Plex | IBM Plex | SAME |

### Action expected by sovereign

The PRIMER-1 is a **draft for ratification**. Until the sovereign issues a 999_SEAL for the new hex values, the existing `design-tokens.json` (version `PRIMER-1.0`, ratified 2026-08-01) **remains canonical**.

Decision needed per I8 (diff before mutation) + F13 (sovereign veto):

- **Option A:** ACCEPT primer — sovereign authorizes migration of `design-tokens.json` to PRIMER-1 hex values. Atlas bumps to ATLAS-2.0.
- **Option B:** KEEP existing — sovereign rejects primer's hex values; the primer becomes a documentary note only, not a token override.
- **Option C:** HYBRID — accept some primer values, keep others (e.g. accept red shift, reject yellow shift). Sovereign specifies which.
- **Option D:** DEFER — keep both, document the diff, surface as U1 unknown for next session.

---

## Appendix B — Atlas compliance

This document respects ATLAS-1.1 invariants:

- **I1 ATLAS before action:** `atlas.json` v1.1 is the supreme map. This primer is `canon/design-primer.md`. Both registered.
- **I2 Canon before code:** PRIMER-1 is the long-form documentary home. `design-tokens.json` is the JSON twin. Both required.
- **I3 SOT before rendered page:** This document is SOT for the primer. The existing `design-tokens.json` is SOT for the ratified token values. Diff surfaced.
- **I8 Diff before mutation:** Appendix A IS the diff. Sovereign ratification pending.
- **I10 Unknowns declared, not hidden:** All discrepancies surfaced as U1 unknown (see Atlas atlas.json).
- **I11 Human owns meaning:** Sovereign has final veto on token values. This primer is a proposal.
- **I12 Agent owns operational clarity:** The diff is structural, not emotional. Hex values are operational truth.
- **I13 No automation without reversibility:** No token values changed yet. Existing canon unchanged. Reversible.
